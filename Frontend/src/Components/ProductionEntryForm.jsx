  ...
  const handleSubmit = async (e) => {
    ...
    for (let plan of productionPlans) {
      ...
      if (isNaN(actual) || actual < 0 || actual > plan.plannedProductionUnits) {
        toast.error(
          `Please enter a valid number for ${plan.vehicleModel} - ${plan.shift}.`
        );
        return;
      }
      const rate = calculateProductionRate(plan.plannedProductionUnits, actual);
      if (rate < 70 && !anomalies[`${plan.shift}-${plan.vehicleModel}`]) {
        toast.error(
          `Please select an anomaly type for ${plan.vehicleModel} - ${plan.shift}.`
        );
        return;
      }
    }
    ...
    const submissions = productionPlans.map((plan, index) => {
      ...
      const rate = calculateProductionRate(plan.plannedProductionUnits, actual);
      const anomaly =
        rate < 70 ? anomalies[`${plan.shift}-${plan.vehicleModel}`] : null;
        ...
        amountLoss:
          rate < 70
            ? ((plan.plannedProductionUnits -
                Number(actualUnits[plan.shift][plan.vehicleModel])) *
      ...
      {!showForm && (
        ...
        >
          <h3 className="text-2xl font-semibold mb-6">
            Enter Actual Production Units for Today
          </h3>
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-red-600 font-semibold">
                          ...
                          {(
                            ((plan.plannedProductionUnits -
                              Number(
                                actualUnits[plan.shift][plan.vehicleModel]
                              )) *
                              vehicle.productionPrice) /
                            2
                          ).toLocaleString()}
                  ...
                  {showAnomaly && (
                    <div className="mt-6">
                      ...
                      >
                        ...
                        <option value="Electrical Failure">
                          Electrical Failure
                        </option>
                        <option value="Supply Chain Issues">
                          Supply Chain Issues
                        </option>
                        <option value="Labor Shortage">Labor Shortage</option>
                        <option value="Material Shortage">
                          Material Shortage
                        </option>
          ...
          {totalLoss > 0 && (
            <div className="mt-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg">
              <p className="font-semibold">
                Total Loss Due to Low Production Rates: ₹
