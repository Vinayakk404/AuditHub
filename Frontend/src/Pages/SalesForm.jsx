...
const API_URL =
  "http://ec2-54-171-48-97.eu-west-1.compute.amazonaws.com:8080/api/"; // Adjust the backend API base URL
...
const SalesForm = () => {
  ...
  const handleVehicleIdChange = (e) => {
    ...
    if (selectedVehicleId) {
      getInventoryByVehicleId(selectedVehicleId)
        .then((response) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "quantitySold") {
      if (value > totalVehicles) {
        toast.error(
          `Cannot sell ${value} vehicles. Only ${totalVehicles} available.`
        );
        saleData.value = 0;
        return;
      }
  ...
  const handleSubmit = (e) => {
    ...
    if (quantityToSell > totalVehicles) {
      toast.error(
        `Cannot sell ${quantityToSell} vehicles. Only ${totalVehicles} available.`
      );
        ...
        {totalVehicles > 0 ? (
          <div className="mb-6 text-green-600">
            <h2 className="text-lg font-semibold">
              Total Vehicles Available: {totalVehicles}
            </h2>
          </div>
        ) : (
          <>
